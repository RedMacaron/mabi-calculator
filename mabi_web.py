...
> **그랜마 상인 + 알파카 + 임프의 고급 보증서(필수!) >>> 탈틴 or 티르코네일 판매** > **탈농템은 완제 해금 해놓고 배,석영,거미줄,재스민 위주로 에너지 다써서 단축시키고 수확만! 그 이상은 시간아까움** """)

# 스트림릿 탭 기능을 사용하여 기존/신규 분리
tab1, tab2 = st.tabs(["📚 기존 표준 루트 (6T 위주)", "🛠️ 신규 커스텀 루트 (P/C 6T 제외)"])
